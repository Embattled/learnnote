...
- [2. Declaration 声明](#2-declaration-声明)
  ...
  - [Array declaration 数组的声明](#array-declaration-数组的声明)
- [3. C 表达式](#3-c-表达式)
  - [3.1. 一般概念](#31-一般概念)
...
- [5. C语言指针](#5-c语言指针)
  - [5.1. 野指针](#51-野指针)
  - [5.2. 指针的定义](#52-指针的定义)
    - [5.2.1. 数组指针 数组名](#521-数组指针-数组名)
    - [5.2.2. 数组和指针的可交换性 下标 \[\]](#522-数组和指针的可交换性-下标-)
    - [5.2.3. C语言字符串](#523-c语言字符串)
    - [5.2.4. 函数中的指针](#524-函数中的指针)
    - [5.2.5. NULL 指针](#525-null-指针)
    - [5.2.6. void 指针](#526-void-指针)
    - [5.2.7. 函数指针](#527-函数指针)
- [6. 预处理命令](#6-预处理命令)
  - [6.1. include](#61-include)
  - [6.2. define 宏定义](#62-define-宏定义)
    - [6.2.1. 带参数的宏定义](#621-带参数的宏定义)
    - [6.2.2. 特殊宏定义](#622-特殊宏定义)
  - [6.3. 条件宏](#63-条件宏)
    - [6.3.1. if](#631-if)
    - [6.3.2. ifdef](#632-ifdef)
  - [6.4. error 报错停止](#64-error-报错停止)
  - [6.5. 预定义宏](#65-预定义宏)
    - [6.5.1. 平台](#651-平台)
    - [6.5.2. 编译模式](#652-编译模式)
- [7. 多文件编程](#7-多文件编程)
  - [7.1. 从 extern 开始](#71-从-extern-开始)
  - [7.2. 防止重复包含头文件](#72-防止重复包含头文件)
  - [7.3. 命名空间](#73-命名空间)
  - [7.4. const 在多文件编程中](#74-const-在多文件编程中)
  - [7.5. 头文件应该写什么](#75-头文件应该写什么)
- [8. C语言和内存](#8-c语言和内存)
  - [8.1. 操作系统基础](#81-操作系统基础)
    - [8.1.1. 虚拟内存](#811-虚拟内存)
    - [8.1.2. 编译模式](#812-编译模式)
    - [8.1.3. 地址对齐](#813-地址对齐)
    - [8.1.4. 内存分页](#814-内存分页)
    - [8.1.5. MMU](#815-mmu)
  - [8.2. C语言的内存布局](#82-c语言的内存布局)
    - [8.2.1. 内核空间](#821-内核空间)
    - [8.2.2. Linux 的内存模型](#822-linux-的内存模型)
    - [8.2.3. Windows 的内存模型](#823-windows-的内存模型)
    - [8.2.4. 进程与内存](#824-进程与内存)
    - [8.2.5. 内核模式与用户模式](#825-内核模式与用户模式)
  - [8.3. 栈与进程 栈帧](#83-栈与进程-栈帧)
    - [8.3.1. 栈与函数 调用惯例](#831-栈与函数-调用惯例)
    - [8.3.2. 栈溢出的本质](#832-栈溢出的本质)
  - [8.4. 堆与进程 动态内存](#84-堆与进程-动态内存)
    - [8.4.1. 堆与内存池](#841-堆与内存池)
    - [8.4.2. 堆与内存泄漏 Memory Leak](#842-堆与内存泄漏-memory-leak)
- [9. C语言多线程](#9-c语言多线程)
  - [9.1. Linux POSIX 标准](#91-linux-posix-标准)
    - [9.1.1. C 语言下的编译](#911-c-语言下的编译)
    - [9.1.2. 线程操作](#912-线程操作)
    - [9.1.3. 4种线程同步方法](#913-4种线程同步方法)
    - [9.1.4. 线程属性](#914-线程属性)
  - [9.2. Windows 的POSIX](#92-windows-的posix)
  - [9.3. C++11 的多线程](#93-c11-的多线程)
- [10. dlfcn.h  显示调用动态链接库](#10-dlfcnh--显示调用动态链接库)
