...
int flipVertical(mat *source)
{
    ...
    int *newData = new (std::nothrow) int[srcValueNumber];
    if (newData == nullptr)
    {
...
int bilinearInterpolation(const mat *source, mat *destination, double times)
{
    ...
    double *xCeilWeight = new double[newWidth];
    double *xFloorWeight = new double[newWidth];
    ...
    double *xInterpolation = new double[newWidth * oldHigh]{0};
    ...
    double *yCeilWeight = new double[newHigh];
    double *yFloorWeight = new double[newHigh];
    for (int i = 1; i <= newHigh; i++)
    {
    ...
    double *addBuffer = new double[valueNumber]{0};
    for (int row = 0; row < newHigh; row++)
    {
        long long rowIndex = row * newWidth;
...
int convolution(const mat *source, mat *destination, int core[9])
{
    ...
    int width = source->getWidth();
    int hight = source->getHigh();
    destination->initialize(format, width, hight, source->getColordepth());
    ...
    for (int y = 0; y < hight; y++)
    {
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            if (needNormalizing)
            {
                destination->ascContent[rowOffset + x] = (int)round(destination->ascContent[rowOffset + x] / (double)abs(scaleNum));
            }
        }
    }
    return RYU_SUCCESS;
